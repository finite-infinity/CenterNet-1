...
def _left_aggregate(heat):
    ...
    '''
    shape = heat.shape  # b c h w
    heat = heat.reshape(-1, heat.shape[3])  
    heat = heat.transpose(1, 0).contiguous()  # w b*c*h
    ret = heat.clone()
    for i in range(1, heat.shape[0]):
        inds = (heat[i] >= heat[i - 1])  #判断右>左
        ret[i] += ret[i - 1] * inds.float()  #积累真值
...
def _top_aggregate(heat):
    '''
        heat: batchsize x channels x h x w
    '''
    heat = heat.transpose(3, 2)   # b c h w
    shape = heat.shape
    heat = heat.reshape(-1, heat.shape[3])
    heat = heat.transpose(1, 0).contiguous()
    ret = heat.clone()
...
def ctdet_decode(heat, wh, angle, reg=None, cat_spec_wh=False, K=100):
    batch, cat, height, width = heat.size()
    ...
    if reg is not None:
      reg = _transpose_and_gather_feat(reg, inds)
      reg = reg.view(batch, K, 2)  # reshape
      xs = xs.view(batch, K, 1) + reg[:, :, 0:1]
      ys = ys.view(batch, K, 1) + reg[:, :, 1:2]  # 加残差，提高精度
    else:
      xs = xs.view(batch, K, 1) + 0.5
      ys = ys.view(batch, K, 1) + 0.5
    wh = _transpose_and_gather_feat(wh, inds)
    angle = _transpose_and_gather_feat(angle, inds)
    if cat_spec_wh:  # wh添加class项
    ...
    bboxes = torch.cat([xs - wh[..., 0:1] / 2, 
                        ...
                        xs + wh[..., 0:1] / 2, 
                        ys + wh[..., 1:2] / 2], dim=2)
    detections = torch.cat([bboxes, scores, clses, angle], dim=2)
