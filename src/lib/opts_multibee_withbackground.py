...
class opts(object):
  def __init__(self):
    ...
    self.parser.add_argument('--vis_thresh', type=float, default=0.1,   # 另一个0.6
    ...
    self.parser.add_argument('--not_rand_crop', action='store_true',
                             help='not use the random crop data augmentation'
                                  'from CornerNet.')
    self.parser.add_argument('--shift', type=float, default=0.01,
  ...
  def init(self, args=''):
    default_dataset_info = {
      'ctdet': {'default_resolution': [2048, 2048], 'num_classes': 2,    #是2？
                'mean': [0.408, 0.447, 0.470], 'std': [0.289, 0.274, 0.278],
                'dataset': 'coco_multibee'},
