...
class opts(object):
  def __init__(self):
    ...
    self.parser.add_argument('--load_model', default='',
                             help='path to pretrained model')  #预训练模型
    self.parser.add_argument('--resume', action='store_true',  #继续训练
    ...
    self.parser.add_argument('--num_workers', type=int, default=4,
                             help='dataloader threads. 0 for single-thread.') #多线程
    ...
    self.parser.add_argument('--head_conv', type=int, default=-1,
                             help='conv layer channels for output head'
                                  ...
                                  '64 for resnets and 256 for dla.')     # output层输入channel
    self.parser.add_argument('--down_ratio', type=int, default=4,
                             help='output stride. Currently only supports 4.')   # 下采样参数

    # input
    self.parser.add_argument('--input_res', type=int, default=-1, 
                             help='input height and width. -1 for default from '
                             'dataset. Will be overriden by input_h | input_w') # 输入w=h=res
    ...
    self.parser.add_argument('--val_intervals', type=int, default=10,
                             help='number of epochs to run validation.')  #推断间隔
    ...
    self.parser.add_argument('--fix_res', action='store_true',
                             help='fix testing resolution or keep '
                                  'the original resolution')  # fix测试分辨率或保持
    self.parser.add_argument('--keep_res', action='store_true',
                             help='keep the original resolution'
                                  ' during validation.')    # 验证集保持分辨率
  ...
  def parse(self, args=''):
    ...
    if opt.resume and opt.load_model == '':
      print('*'*50+'it is resume from the saved model')
  ...
  def update_dataset_info_and_set_heads(self, opt, dataset):
    ...
    opt.output_w = opt.input_w // opt.down_ratio   # 下采样
    opt.input_res = max(opt.input_h, opt.input_w)
    opt.output_res = max(opt.output_h, opt.output_w)
    
    #task：exdet：extremenet目标检测 ctdet：centernet目标检测
    # ddd：3d_box检测   multi_pose：姿态检测
    if opt.task == 'exdet':
      ...
      # assert opt.dataset in ['pascal', 'coco'] 添加角度
