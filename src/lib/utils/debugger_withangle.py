...
class Debugger(object):
  def __init__(self, ipynb=False, theme='black', 
    ...
    elif dataset == 'coco_gatebee' or dataset == 'coco_multibee'or dataset == 'coco_multibee_withangle':
      self.names = bee_class_name
      #print('*'*50+'it is the bee dataset')      
    elif dataset == 'gta':
  ...
  def add_points(self, points, color, img_id='default'):
      c = color
      cv2.circle(self.imgs[img_id], (int(points[0] * self.down_ratio), 
                                      int(points[1] * self.down_ratio)),
                   5, (255, 255, 255), -1)
      cv2.circle(self.imgs[img_id], (int(points[0] * self.down_ratio),
                                      int(points[1] * self.down_ratio)),
                   3, (int(c[0]), int(c[1]), int(c[2])), -1)

  def add_arrow(self, point, angle, class_id, w=1, d=25.0, img_id='default'):
    (x1,y1,x2,y2) = (0,0,0,0)
    dn = 4
    c = self.colors[class_id, 0, 0]
    ...
    if class_id == 0:
      ...
      cv2.line(self.imgs[img_id], (x,y), (x2,y2), (0,255,255), w,
                        lineType=cv2.LINE_AA)
      
    cv2.circle(self.imgs[img_id], (x, y),
                5, (255, 255, 255), -1)
    cv2.circle(self.imgs[img_id], (x, y),
                3, (int(c[0]), int(c[1]), int(c[2])), -1)
  ...
  def add_text(self, textinfo, img_id='default'):
    image = self.imgs[img_id]
    cv2.putText(image, text=textinfo, org=(50, 70), fontFace=cv2.FONT_HERSHEY_SIMPLEX,
            fontScale=1, color=(0, 0, 255), thickness=2)                

  def save_img(self, imgId='default', path='./output/withangle/', imgname='default'):
    print('save img to %s'%(path+imgname))
    cv2.imwrite(path + imgname, self.imgs[imgId])


  def get_img_result(self, imgId='default'):
