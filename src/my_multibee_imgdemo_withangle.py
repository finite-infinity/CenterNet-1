...
from opts_multibee import opts
from detectors.detector_factory_withangle import detector_factory
import numpy as np
...
def demo(opt):
  ...
  if opt.demo == 'webcam':
    ...
    for (image_name) in image_names:
      with open('./output/withangle/'+ os.path.basename(image_name)[:-4]+'.txt', 'w') as f:   
